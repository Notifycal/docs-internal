# ESTRUCTURA - POLÍTICA DE PRIVACIDAD NOTIFYCAL

## VARIABLES CONFIGURABLES
```
EMPRESA:
- Razón social: [PENDIENTE]
- Nombre comercial: Notifycal
- NIF/CIF: [PENDIENTE]
- Domicilio: [PENDIENTE]
- CP y Localidad: [PENDIENTE]

CONTACTO:
- Email privacidad: privacidad@notifycal.com
- Email soporte: soporte@notifycal.com
- Email bajas: baja@notifycal.com
- Web: www.notifycal.com

ACTUALIZACIÓN:
- Fecha última actualización: 11 de agosto de 2025
- Versión: 1.0
```

## 1. INFORMACIÓN GENERAL
- Ámbito de aplicación
- Definiciones (Usuario, Destinatario, Servicio)
- Roles en tratamiento (Responsable vs Encargado)

## 2. DATOS QUE RECOPILAMOS
### 2.1. De Usuarios (nuestros clientes)
- Datos del negocio (nombre, dirección, sector, tamaño)
- Configuración calendarios y plantillas
- Stripe Customer ID y créditos
- Fechas aceptación términos

### 2.2. De Destinatarios (clientes de usuarios)
- Solo datos del calendario
- Nombre, teléfono, email, fecha cita
- NO recopilación directa

### 2.3. Datos técnicos
- Navegación
- Analytics (Clarity, GA)

## 3. BASES LEGALES Y FINALIDADES
### 3.1. Para Usuarios
- Tabla: Finalidad | Base legal | Descripción

### 3.2. Para Destinatarios
- Como encargados del tratamiento

## 4. COOKIES Y ALMACENAMIENTO LOCAL
- No cookies propias (ni siquiera para sesión)
- Solo analytics de terceros (con consentimiento)
- LocalStorage para autenticación y UX

## 5. COMPARTICIÓN CON TERCEROS
### 5.1. Proveedores (Encargados)
- Tabla: Tipo proveedor | Servicio | Datos | Ubicación

### 5.2. Transferencias internacionales
- Cláusulas contractuales tipo
- Garantías adicionales

## 6. SEGURIDAD
- Medidas técnicas (cifrado, OAuth, cloud computing)
- Copias de seguridad
- No almacenamiento datos financieros
- Medidas organizativas
- Notificación brechas

## 7. CONSERVACIÓN DE DATOS
- Tabla: Tipo datos | Plazo | Justificación
- 6 meses logs recordatorios y eventos
- 5 años facturación
- 1 año logs técnicos
- Eliminación automática

## 8. DERECHOS INTERESADOS
- Lista de derechos RGPD
- Cómo ejercerlos
- Procedimiento y plazos
- Reclamación AEPD

## 9. OBLIGACIONES USUARIOS
- Responsabilidades como responsable
- Contrato encargo tratamiento
- Garantías sobre datos destinatarios

## 10. MENORES DE EDAD
- No dirigido a menores 14 años
- Responsabilidad del usuario

## 11. CAMBIOS EN LA POLÍTICA
- Procedimiento actualización
- Notificación según impacto

## 12. INFORMACIÓN ADICIONAL
- Enlaces terceros
- Do Not Track
- Privacidad por diseño

## 13. CONTACTO
- Datos responsable (usar variables)
- Tiempo respuesta

## 14. GLOSARIO
- Términos técnicos y legales